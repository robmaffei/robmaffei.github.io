...
function displayImage(container, path, caption) {
    ...
    if (!caption) {  // If notViewed
        ...
        imgElement.style.filter = 'sepia(100%) brightness(.9) opacity(1)';
    }
    ...
    if (caption) {
        ...
        captionElement.style.backgroundColor = 'rgba(255, 255, 255, .8)'; // lightgray with 80% opacity
        captionElement.style.borderTop = '1px solid black'; // Top border
        captionElement.style.borderBottom = '1px solid black'; // Bottom border
    ...
    imgElement.addEventListener('click', function () {
        openFullScreenImage(baseUrl + 'game/' + path);
    });
}
